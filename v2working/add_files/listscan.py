...
def indiv_output():              #individual output for each ip
	os.system("""
	bold=$(tput bold)
	normal=$(tput sgr0)
	...
	echo
	sleep 1
	echo "\e[5m\e[1m\e[91mSCANNING STARTED... \e[92mKEEP PATIENCE..!!!\e[0m"
	sleep 1
	while read ip; do
	#echo "\e[101m \n[+] Testing IP : $ip \e[0m" && nmap -v -sV -d -A -p1-65535 --script vuln,default -T4 -Pn $ip -oA ~/Desktop/$output/$ip
